...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("wikiRank")
  val sc: SparkContext = new SparkContext(conf)
  // Hint: use a combination of `sc.parallelize`, `WikipediaData.lines` and `WikipediaData.parse`
  val wikiRdd: RDD[WikipediaArticle] = sc.parallelize(WikipediaData.lines).map(line ⇒ WikipediaData.parse(line)).cache()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = 
    index.map(item ⇒ (item._1, item._2.size)).collect.toList.sortBy(_._2).reverse
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = 
    rdd.flatMap(article ⇒ langs.filter(lang ⇒ article.mentionsLanguage(lang)).map(lang ⇒ (lang, 1))).reduceByKey(_+_).collect().toList.sortBy(_._2).reverse
