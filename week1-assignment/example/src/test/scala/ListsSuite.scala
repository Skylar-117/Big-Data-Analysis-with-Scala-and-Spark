 ...
 class ListsSuite {
  ...
  @Test def `one plus one is three (0pts)?`: Unit = {
    assert(1 + 1 != 3) // This assertion fails! Go ahead and fix it.
                       // Original code here is "assert(1+1==3)", I changed it to "assert(1+1!=3)", then unit test passed :)     
  }
  ...
  @Test def `details why one plus one is not three (0pts)`: Unit = {
    Assert.assertEquals(3, 1 + 1 + 1) // Fix me, please!
                                      // Original code here is "Assert.assertEquals(3,1+1)", I changed it to "Assert.assertEquals(3,1+1+1)", then unit test passed :)
  }
