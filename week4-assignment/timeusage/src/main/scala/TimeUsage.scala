...
object TimeUsage extends TimeUsageInterface {
  ...
  def timeUsageByLifePeriod(): Unit = {
    val (columns, initDf) = read("src/main/resource/atussum.csv")
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primary = columnNames.filter{
      x ⇒ primaryNeeds.exists(test ⇒ x.startsWith(test))
    }.map(column)
    
    val working = columnNames.filter{
      x ⇒ workingActivities.exists(test ⇒ x.startsWith(test))
    }.map(column)
    
    val other = columnNames.filter{ x ⇒ (otherActivities.exists(test ⇒ x.startsWith(test))) }
                           .filterNot{ x ⇒ (primaryNeeds.exists(test ⇒ x.startsWith(test))) }
                           .filterNot{ x ⇒ (workingActivities.exists(test ⇒ x.startsWith(test))) }
                           .map(column)
  ...
  def timeUsageSummary(
    ...
    def sumColumns(x: List[Column]): Column = x.reduce(_+_).divide(60)
